...
class TimeController {
  public static async postTime(ctx: Context) {
    ...
    // targetDay不存在需要创建一个
    if (!targetDay) {
      console.log(timeMarker, timeMarkerValue, wakeUpTime)
  ...
  public static async postDayList(ctx: Context) {
    ...
    const handleAverage = (daysList) => {
      ...
      let averageSleepTimeResult = '',
        averageWakeUpTimeResult = '',
        averageBedTimeResult = ''
      if (sleepTimeList && sleepTimeList.length) {
        averageSleepTimeResult = (sleepTimeList.reduce((prev, item) => prev + item, 0) / sleepTimeList.length).toFixed(2)
      }
    ...
    if (totalDaysList && totalDaysList.days.length) {
    ...
    if (targetUser) {
      ...
      ctx.body = {
        status: true,
        data: {
          weekDaysList: (weekDaysList && weekDaysList.days) || [],
          monthDaysList: (monthDaysList && monthDaysList.days) || [],
          weekSleepTime,
          weekWakeUpTime,
          weekBedTime,
          monthSleepTime,
